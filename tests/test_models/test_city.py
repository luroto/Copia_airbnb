...
class testCity(unittest.TestCase):
    ''' Test of the model City '''

    @classmethod
    def setUpClass(cls):
        ''' test_inst up '''
        cls.test_inst = City()
        cls.test_inst.name = "springfield"
        cls.test_inst.save()
        cls.test_inst_str = cls.test_inst.to_dict()

    def test_isInstance(self):
        ''' test if is a instance of BaseModel '''
        self.assertIsNot(self.test_inst, City)
        self.assertTrue(isinstance(self.test_inst, City))

    def test_attrib(self):
        ''' test the attributes created '''
        self.assertEqual(self.test_inst_str['name'], 'springfield')
