...
class testState(unittest.TestCase):
    ''' Test of the model State '''

    @classmethod
    def setUpClass(cls):
        ''' test_inst up '''
        cls.test_inst = State()
        cls.test_inst.name = "london"
        cls.test_inst.save()
        cls.test_inst_str = cls.test_inst.to_dict()

    def test_isInstance(self):
        ''' test if is a instance of BaseModel '''
        self.assertIsNot(self.test_inst, State)
        self.assertTrue(isinstance(self.test_inst, State))

    def test_attrib(self):
        ''' test the attributes created '''
        self.assertEqual(self.test_inst_str['name'], 'london')
